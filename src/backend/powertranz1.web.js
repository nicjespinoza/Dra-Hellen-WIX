...
// Estas funciones no se exportan y no necesitan cambios. Son privadas para este módulo.

async function getCredentials() {
    const secretName = 'powertranzCredentials1';
    try {
...
export const initTransaction1 = webMethod(Permissions.SiteMember, async (userId, orderId, amount, billingAddress, sessionId, bypass3DS = false, concept) => {
    try {
        ...
        const response = await retryFetch('https://gateway.ptranz.com/api/spi/sale', {
            method: 'POST',
            headers: {
...
export const reverseTransaction1 = webMethod(Permissions.SiteMember, async (powertranzTransactionId) => {
    ...
    try {
     ...
     // ================== INICIO DEL CAMBIO ==================
        ...
        const response = await retryFetch('https://gateway.ptranz.com/api/spi/refund', { // <-- CAMBIO DE URL
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Powertranz-PowertranzId': powertranzId,
...
export async function completePayment1(spiToken, transactionId, userId, csrfToken, sessionId, powertranzResponse = {}) {
    try {
        ...
        const response = await retryFetch('https://gateway.ptranz.com/api/spi/payment', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json', 
                'Powertranz-PowertranzId': powertranzId,
...
export const getMyTransactions1 = webMethod(Permissions.SiteMember, async () => {
    try {
