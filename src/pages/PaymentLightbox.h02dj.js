    ...
    $w.onReady(function () {
        $w('#htmlEmbed1').hide();
        $w('#statusText').text = 'Formulario de pago activo...';
        $w('#btnCancel').show();
   ...
   function handleFormSubmission(data) {
    // 1. Informamos al usuario que está siendo redirigido.
    $w('#statusText').text = 'Redirigiendo a la pasarela de pago segura...';
    $w('#btnCancel').disable(); // Deshabilitamos el botón para evitar clics accidentales.
    ...
    // 2. ¡LA CLAVE! Cerramos el lightbox después de un pequeño retraso.
    // El retraso es para que el usuario alcance a leer el mensaje "Redirigiendo...".
    setTimeout(() => {
        lightbox.close(); 
    }, 1500); // Cerramos después de 1.5 segundos. Puedes ajustar este tiempo.
}
